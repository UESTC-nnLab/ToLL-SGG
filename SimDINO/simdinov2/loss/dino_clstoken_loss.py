...
class MCRLoss(DINOCenter):
    ...
    def forward(self, student_feat_list, teacher_feat_list, no_diag=True, normalized=True):
        # 一个 list，里面是多个 view 的 student 特征，每个张量形状大致是 (N, D)
        # 一个 list，通常是 teacher 的 global views 特征（SimDINO 里一般是 2 个 global crop），每个也是 (N, D)
        """
        ...
        if not normalized:
            student_feat = F.normalize(student_feat, p=2, dim=-1) # (n_student_views, N, D)  
            # N不是 batch size（图的数量），而是“这次参与 loss 的样本数”，例如边的数量、对象的数量等
            teacher_feat = F.normalize(teacher_feat, p=2, dim=-1) # (n_teacher_views, N, D)

        # 压缩项（对齐项）
        comp_loss, global_comp_loss = self.calc_compression(student_feat, teacher_feat, no_diag=no_diag)
        # 计算 teacher vs student 的余弦相似度（点积，因为默认是 L2 normalize 后）
        # 直觉：让 student 的各个视图输出能“靠近” teacher 的输出（跨视图一致性）
