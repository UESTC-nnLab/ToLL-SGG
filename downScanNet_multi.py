#!/usr/bin/env python
# (MODIFIED FOR ARIA2C PARALLEL DOWNLOAD)
...
try:
    ...
    print("错误: aria2c 未安装或未在 PATH 中。")
    print("请先安装 aria2c 来使用此脚本。")
    print("  Ubuntu/Debian: sudo apt install aria2")
    print("  macOS (Homebrew): brew install aria2")
    sys.exit(1)
...
def download_batch_aria2(file_list):
    """
    使用 aria2c 并行下载一个文件列表 (url, out_path)。
    -c 标志会自动跳过已完成的文件。
    """
    if not file_list:
        print("所有文件均已存在，无需下载。")
    ...
    print(f"准备使用 aria2c 并行下载 {len(file_list)} 个文件...")

    # 创建一个临时的输入文件
    with tempfile.NamedTemporaryFile(mode='w', delete=False) as f:
        for url, out_path in file_list:
            out_dir = os.path.dirname(out_path)
            out_name = os.path.basename(out_path)
            
            # 确保目录存在 (aria2c 不会自动创建多层目录)
    ...
    command = [
        ...
        '-x', '16', # 每个服务器最大 16 个连接
        '-s', '16', # 将文件分割成 16 块下载
        '--summary-interval=5', # 每 5 秒打印一次摘要
        '-i', temp_file_path
    ]

    print(f"已启动 aria2c... URL 列表: {temp_file_path}")
    try:
        subprocess.run(command, check=True)
        print("aria2c 批处理下载完成。")
        os.remove(temp_file_path) # 成功后删除
    except subprocess.CalledProcessError as e:
        print(f"ERROR: aria2c 下载失败。 {e}")
        print(f"临时文件保存在: {temp_file_path}")
        print(f"您可以稍后手动运行以下命令来恢复：")
        print(f"aria2c -c -j 16 -i {temp_file_path}")
...
def download_release(release_scans, out_dir, file_types, use_v1_sens, skip_existing):
    ...
    for scan_id in release_scans:
        ...
        for ft in file_types:
            ...
            out_file = os.path.join(scan_out_dir, scan_id + ft)
            
            if skip_existing and os.path.isfile(out_file):
                continue
            file_list.append((url, out_file))

    # 一次性提交所有文件到 aria2c
    download_batch_aria2(file_list)
    print('ScanNet ' + RELEASE_NAME + ' release 下载任务已提交。')


def download_file(url, out_file):
    """
    修改为使用 aria2c 下载单个文件。
    -c 标志会自动处理“跳过已存在文件”的逻辑。
    """
    ...
    # 我们依赖 aria2c 的 -c 标志来处理文件存在的情况
    # 原始脚本的 `if not os.path.isfile(out_file):` 逻辑被 aria2c -c 完美替代
    
    if os.path.isfile(out_file):
        print('文件已存在，aria2c 将检查完整性: ' + out_file)
    else:
        print('\t' + url + ' > ' + out_file)
    ...
    command = [
        ...
        '--dir', out_dir,
        '--out', out_name,
        '--summary-interval=0', # 单个文件不显示摘要
        '--quiet=true', # 保持安静，除非出错
        url
    ]
    try:
        subprocess.run(command, check=True, stderr=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        print(f"ERROR downloading {url}: {e.stderr.decode()}")
    # 注意：这里不再需要 Fallback，因为脚本开头已经检查了 aria2c


def download_scan(scan_id, out_dir, file_types, use_v1_sens, skip_existing=False):
    print('正在为 scan ' + scan_id + ' 收集文件列表...')
    ...
    # 提交单个 scan 的所有文件到 aria2c
    download_batch_aria2(file_list)
    print('Scan ' + scan_id + ' 下载任务已提交。')
...
def download_task_data(out_dir):
    ...
    for file in files:
        ...
        download_file(url, localpath) # 这将使用新的 aria2c download_file
    ...
    for folder_name in ['hires_tfrecords', 'lores_tfrecords']:
        ...
        for split, num_shards in split_to_num_shards.items():
            for i in range(num_shards):
                ...
                # 在这里不检查 skip_existing，因为tfrecords太大了
                # 我们完全依赖 aria2c 的 -c 标志
                file_list.append((url, localpath))
                
    # 批量下载所有 tfrecords
    download_batch_aria2(file_list)
    print('TF records 下载任务已提交。')
...
def main():
    ...
    args = parser.parse_args(['-o', '/home/honsen/tartan/ScanNet','--skip_existing']) #, '--id', 'scene0007_00'
    ...
    if args.task_data:  # download task data
        ...
        print('Note that aria2c will automatically skip fully downloaded files.')
        print('***')
        print('Press any key to continue, or CTRL-C to exit.')
        try:
            key = input('')
        ...
        if not args.v1:
            ...
            # --- MODIFIED CALL for test scans ---
            print("\nCollecting files for TEST scans...")
            ...
            # --- These are single files, download_file will handle them ---
            print("\nDownloading auxiliary task files...")
            download_file(os.path.join(BASE_URL, RELEASE_TASKS, TEST_FRAMES_FILE[0]), os.path.join(out_dir_tasks, TEST_FRAMES_FILE[0]))
            for k in range(len(DATA_EFFICIENT_FILES) - 1):
                download_file(os.path.join(BASE_URL, RELEASE_TASKS, DATA_EFFICIENT_FILES[k]), os.path.join(out_dir_tasks, DATA_EFFICIENT_FILES[k]))
