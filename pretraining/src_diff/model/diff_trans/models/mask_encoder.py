...
class Mask_Encoder(nn.Module):
    ...
    def forward(self, neighborhood, center, mask_ratio=None, noaug = False):
        ...
        visible_center = center[~bool_masked_pos].reshape(batch_size, -1, 3)
        pos = self.pos_embed(visible_center)
        x_vis = self.blocks(x_vis, pos)
        x_vis = self.norm(x_vis)
        group_input_tokens[~bool_masked_pos] = x_vis.reshape(-1, self.trans_dim)
        encoder_token = group_input_tokens
        return encoder_token, bool_masked_pos, x_vis, visible_center
