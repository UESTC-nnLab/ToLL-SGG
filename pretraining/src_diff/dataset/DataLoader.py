...
def collate_fn_mmg_diff_mv(batch):
    # batch
    cur_obj_texts = []
    obj_point_list = []
    edge_indices, descriptor = [], []
    ...
    for i, b in enumerate(batch):
        ...
        edge_indices.append(b[3].long() + count)
        ...
        # get batchs location
        batch_ids.append(torch.full((b[0].shape[0], 1), i, dtype=torch.long))
...
def collate_fn_mmg_diff(batch):
    ...
    for i, b in enumerate(batch):
        ...
        # Support both orders:
        # 1) (..., descriptor, edge_indices, ...)
        # 2) (..., edge_indices, descriptor, ...)
        if third_item.dim() == 2 and third_item.shape[-1] == 11:
            cur_descriptor = third_item
            cur_edge_indices = fourth_item
        else:
            cur_edge_indices = third_item
            cur_descriptor = fourth_item

        obj_point_list.append(b[0])
        obj_points_spatial.append(b[1])
        descriptor.append(cur_descriptor)
        edge_indices.append(cur_edge_indices.long() + count)
        anchor_ids.append(b[4])
        cur_obj_texts+=b[5]
        if len(b) > 6 and b[6] is not None and torch.is_tensor(b[6]) and b[6].ndim == 1:
            obj_class_labels.append(b[6])
        if len(b) > 7 and b[6] is not None and torch.is_tensor(b[6]) and b[6].ndim == 1 and b[7] is not None and torch.is_tensor(b[7]):
            edge_class_labels.append(b[7])
        if len(b) > 6 and b[6] is not None and torch.is_tensor(b[6]) and b[6].ndim == 2:
            atlas_embeddings.append(b[6])
        if len(b) > 7 and b[6] is not None and torch.is_tensor(b[6]) and b[6].ndim == 2 and b[7] is not None and torch.is_tensor(b[7]):
            atlas_valid_masks.append(b[7])
        # accumulate batch number to make edge_indices match correct object index
        count += b[0].shape[0]
    ...
    return  torch.cat(obj_point_list, dim=0), torch.cat(obj_points_spatial, dim=0),\
            ...
            cur_obj_texts, torch.cat(batch_ids, dim=0), \
            (torch.cat(obj_class_labels, dim=0) if len(obj_class_labels) > 0 else None), \
            (torch.cat(edge_class_labels, dim=0) if len(edge_class_labels) > 0 else None), \
