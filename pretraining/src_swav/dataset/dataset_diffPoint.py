...
class PdiffDatasetGraph(data.Dataset):
    ...
    def _stable_label_to_int(self, label_name):
    ...
    def _stable_seed(self, *parts):
        ...
        return int(zlib.adler32(key) & 0xFFFFFFFF)

    def _sample_object_points(self, object_points, target_num, sample_id, instance_id, tag):
        if len(object_points) == 0:
            raise ValueError(f"Empty object points for sample={sample_id}, instance={instance_id}")
        rng = np.random.default_rng(self._stable_seed(sample_id, instance_id, tag))
        choice = rng.choice(len(object_points), target_num, replace=True)
        return object_points[choice, :]

    def _load_scene_object_labels(self, cur_scene_path, all_nodes_cur):
        object_labels_path = os.path.join(cur_scene_path, "sensorsData", "object_labels.json")
        if not os.path.exists(object_labels_path):
            ...
            cur_obj_texts = [obj_labels_data.get(str(i), "unknown") for i in all_nodes_cur]

        obj_label_ids = torch.tensor(
            [self._stable_label_to_int(label) for label in cur_obj_texts],
            dtype=torch.long,
        )
        return cur_obj_texts, obj_label_ids
    ...
    def __getitem__(self, index):
        ...
        cur_obj_texts, obj_label_ids = self._load_scene_object_labels(curScenePath, all_nodes_cur)
        ...
        obj_points, obj_points_spatial, edge_indices_view1, descriptor, \
                obj_points_view2, descriptor_view2 = \
            self.data_preparation_multi_view(points, instances, self.num_points, self.num_points_union, all_nodes_cur, selected_rels,
                                  sample_id=sample_id,
                                  padding=0.2)
        
        while (len(edge_indices_view1) == 0) and self.for_train:
            index = np.random.randint(self.__len__())
            obj_points, obj_points_spatial, descriptor, edge_indices_view1, anchor_index, \
                obj_points_view2, descriptor_view2, cur_obj_texts, obj_label_ids, atlas_embeddings, atlas_valid_mask = self.__getitem__(index)

        return obj_points, obj_points_spatial, descriptor, edge_indices_view1, anchor_index, \
                obj_points_view2, descriptor_view2, cur_obj_texts, obj_label_ids, atlas_embeddings, atlas_valid_mask
    ...
    def data_preparation_multi_view(self, points, instances, num_points, num_points_union, all_nodes_cur, selected_rels, scene_id="",
        ...
        for i, instance_id in enumerate(all_nodes_cur):
            ...
            obj_pointset_v1 = self._sample_object_points(
                obj_pointset_raw,
                num_points,
                sample_id if sample_id else scene_id,
                instance_id,
                "view1_spatial",
            )
        ...
        if len(view2_raw_points_buffer) > 0:
            ...
            for i in range(num_objects):
                ...
                # View 2: 保留全局增广后的相对布局，不再做 object 级零均值化
        ...
        # # View 2 使用采样后的边 (保持连通，~50% 数量)
        # # 返回的是 selected_rels 列表中的索引 [0, 2, 5, ...]
        # view2_sel_indices, overlap_sel_indices = self.sample_connected_subgraph(
        #     arrangeidx, selected_rels, keep_ratio=0.5
        # )
        
        # # 将 selected_rels 中的索引转换为模型需要的 node pair 索引
        # # 我们先从 selected_rels 提取出 View 2 的边列表
        # selected_rels_view2 = [selected_rels[i] for i in view2_sel_indices.tolist()]
        
        # # 再转换成 (N, 2) 的 tensor
        # edge_indices_view2 = self.subs_rel_idx(arrangeidx, selected_rels_view2)
        # edge_indices_view2 = torch.tensor(edge_indices_view2, dtype=torch.long)
        
        # 至于 "重合边的索引"，通常在对比学习 Loss 中，我们需要知道 View 2 的第 k 条边对应 View 1 的哪条边。
        # 因为我们是从 selected_rels 中采样的，overlap_sel_indices 记录的就是 View 2 的边在 View 1 (全集) 中的位置 ID。
        # 比如 overlap_sel_indices = [0, 5, 8]，意思是 View 2 的第 0 条边是原图的第 0 条，View 2 的第 1 条是原图的第 5 条...
