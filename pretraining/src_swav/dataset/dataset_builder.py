...
def build_pretrain_dataset(config, for_train=True):
    ...
    atlas_embedding_path = getattr(config, "ATLAS_EMBEDDING_PATH", None) if bool(getattr(config, "ATLAS_ALIGN_ENABLED", False)) else None
    ...
    if pretrain_dataset == "3dssg":
        ...
        return SSGPretrainDatasetGraph(
            ...
            num_points=getattr(dataset_cfg, "num_points", 1024),
...
def build_dataset_for_clustering(config=None):
    ...
    dataset = SSGDatasetGraph(
        split='validation_scans', # 'train_scans' validation_scans
...
if __name__ == '__main__':
    ...
    for i in range(3000):
        obj_points, obj_points_spatial, descriptor, edge_indices_view1, anchor_index, \
                obj_points_view2, descriptor_view2, cur_obj_texts = dataset[i]
        print
