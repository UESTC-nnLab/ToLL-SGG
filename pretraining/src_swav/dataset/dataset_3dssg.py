...
class SSGDatasetGraph(data.Dataset):
    def __init__(self,
        ...
        self.classNames, self.relationNames, data, selected_scans = \
            dataset_loading_3RScan(self.root, self.root, split)
        ...
        self.w_cls_obj = torch.from_numpy(np.array(o_obj_cls)).float().to("cuda")
        self.w_cls_rel = torch.from_numpy(np.array(o_rel_cls)).float().to("cuda")
    ...
    def __getitem__(self, index):
        ...
        data = load_mesh(path, 'labels.instances.align.annotated.v2.ply', self.use_rgb, self.use_normal)

        points = data['points']

        instances = data['instances']

        obj_points, gt_rels, gt_class, edge_indices, descriptor = \
            self.data_preparation(points, instances, 128, 256,
        ...
        while(gt_rels.sum()==0) and self.for_train:
            index = np.random.randint(self.__len__())
            if self.use_descriptor:
                    obj_points, gt_class, gt_rels, edge_indices, descriptor, scan_id = self.__getitem__(index)
            else:
                obj_points, gt_class, gt_rels, edge_indices = self.__getitem__(index)

        if self.use_descriptor:
            return obj_points, gt_class, gt_rels, edge_indices, descriptor, scan_id

        return obj_points, gt_class, gt_rels, edge_indices
    ...
    def data_preparation(self, points, instances, num_points, num_points_union, scene_id="",
                     ...
                     padding=0.2, num_max_rel=-1, shuffle_objs=True, all_edge=False, use_2d_feats=False): #points: all point coord in the scene, instances: the label of each point
        # get instance list
        ...
        return obj_points, gt_rels, label_node, edge_indices, descriptor
