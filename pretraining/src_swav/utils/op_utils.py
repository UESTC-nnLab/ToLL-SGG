...
class Gen_edge_descriptor(nn.Module):
    """
    Computes edge descriptors based on the features of their source and target nodes.
    """
    def __init__(self):
        super().__init__()
    
    def forward(self, descriptor: torch.Tensor, edges_indices: Adj) -> torch.Tensor:
        """
        Args:
            descriptor (Tensor): Node feature matrix with shape [num_nodes, num_node_features].
            edges_indices (Tensor): Graph connectivity in COO format with shape [2, num_edges].

        Returns:
            Tensor: Edge feature matrix with shape [num_edges, num_node_features, 1].
        """
        # Unpack the source (j) and target (i) node indices from the edge_index tensor.
        # For an edge (j, i), j is the source and i is the target.
        source_idx, target_idx = edges_indices[: , 0], edges_indices[:, 1]
        
        # Gather the feature vectors for all source and target nodes.
        x_j = descriptor[source_idx]
        x_i = descriptor[target_idx]
        
        # --- This is the same logic from your original `message` function ---
        
        # Allocate memory for the new edge features.
        ...
        # Add a dimension at the end, as in your original code.
