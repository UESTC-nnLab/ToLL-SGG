...
from src.model.diff_trans.models.PointDif_dino_ori import PointDif
...
from src.model.diff_trans.models.clustering import cluster_and_visualize, visualize_with_gt, evaluate_and_plot_clustering,visualize_aesthetic_gt
...
class Pdiff4SSG_Pretraining():
    ...
    def data_processing_train_pdiff(self, items):
        ...
        anchor_ids, obj_points_view2, descriptor_view2, cur_obj_texts, batch_ids = items
        
        obj_points = obj_points.permute(0, 2, 1).contiguous()
        # [DDP] 使用 self.cuda 辅助函数移动到当前设备
        obj_points, edge_indices, descriptor, batch_ids, obj_points_spatial, obj_points_view2, descriptor_view2 = \
            self.cuda(obj_points, edge_indices, descriptor, batch_ids, obj_points_spatial, obj_points_view2, descriptor_view2)

        return obj_points, descriptor, edge_indices, anchor_ids, batch_ids,\
            obj_points_spatial, obj_points_view2, descriptor_view2, cur_obj_texts
    ...
    def validation(self):
        ...
        model_dicts = torch.load("/home/hyc/hyc_work/sceneGraph/SGG_DIR/save_path/model_epoch_150.pth")
        ...
        for batch_idx, items in enumerate(loader):
            ...
            obj_points_spatial, obj_points_view2, descriptor_view2, cur_obj_texts = self.data_processing_train_pdiff(items)
        
            with torch.no_grad():
                gcn_edge_feature_3d, gcn_obj_feature_3d = self.model.forward_ori(obj_points.permute(0,2,1).contiguous(), edge_indices, obj_points_spatial, descriptor=descriptor, batch_ids=batch_ids, anchor_id=anchor_ids, istrain=False)
        ...
        cluster_and_visualize(all_obj_feats, 200, title_prefix="Object Features",\
    ...
    def validation_for_cls(self, epoch=None):
        ...
        if epoch is not None:
            model_dicts = torch.load(f"/home/hyc/hyc_work/sceneGraph/SGG_DIR/save_path/no_spatial/model_epoch_{epoch}.pth")
        else:
        ...
        vis_save_dir = "/home/hyc/hyc_work/sceneGraph/SGG_DIR/cls_old"
        ...
        visualize_with_gt(all_edge_feats, all_gt_rel_cls, title_prefix="Edge Features",\
                              save_path=os.path.join(vis_save_dir, f"edge_features_cluster_{epoch}.png")) #target_classes=[1,2,12,13,14,16,18]
