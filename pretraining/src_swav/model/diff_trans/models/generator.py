...
class CPDM(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.denoiser_residual = bool(getattr(generator_config, 'denoiser_residual', True))
        self.condition_dropout_prob = float(getattr(generator_config, 'condition_dropout_prob', 0.0))
        self.condition_force_weight = float(getattr(generator_config, 'condition_force_weight', 0.0))
        self.condition_force_margin = float(getattr(generator_config, 'condition_force_margin', 0.0))
        self.condition_force_mode = getattr(generator_config, 'condition_force_mode', 'shuffle')
        self.var_sched = VarianceSchedule(config)
        self.condition_force_start_t = int(
            getattr(generator_config, 'condition_force_start_t', self.var_sched.num_steps + 1)
        )
        self.net = DenoisingNet(point_dim=3, cond_dims=512, residual=self.denoiser_residual)
        self.interval_nums = self.config.generator_config.interval_nums

    def _maybe_apply_condition_dropout(self, cond):
        if (not self.training) or self.condition_dropout_prob <= 0.0:
            return cond

        keep_mask = (
            torch.rand((cond.shape[0], 1), device=cond.device) > self.condition_dropout_prob
        ).to(cond.dtype)
        return cond * keep_mask

    def _build_corrupted_condition(self, cond):
        if cond.shape[0] <= 1 or self.condition_force_mode == 'zero':
            return torch.zeros_like(cond)
        if self.condition_force_mode == 'shuffle':
            perm = torch.randperm(cond.shape[0], device=cond.device)
            if torch.equal(perm, torch.arange(cond.shape[0], device=cond.device)):
                perm = torch.roll(perm, shifts=1)
            return cond[perm]
        raise ValueError(f'Unsupported condition_force_mode: {self.condition_force_mode}')

    def _compute_condition_force_loss(self, noised_coords, beta, cond, noise, pred_noise, t_tensor):
        if self.condition_force_weight <= 0.0:
            return pred_noise.new_zeros(())

        force_mask = t_tensor >= self.condition_force_start_t
        if not torch.any(force_mask):
            return pred_noise.new_zeros(())

        full_mse = F.mse_loss(pred_noise[force_mask], noise[force_mask], reduction='none').mean(dim=(1, 2))
        corrupted_cond = self._build_corrupted_condition(cond)
        with torch.no_grad():
            bad_pred_noise = self.net(
                noised_coords[force_mask],
                beta=beta[force_mask],
                cond=corrupted_cond[force_mask],
            )
            bad_mse = F.mse_loss(bad_pred_noise, noise[force_mask], reduction='none').mean(dim=(1, 2))

        return F.relu(self.condition_force_margin + full_mse - bad_mse.detach()).mean()
    ...
    def get_loss(self, coords, cond, weights=None, ts=None):
        ...
        for i in range(self.interval_nums):
            ...
            # 加噪
            noised_coords = sqrt_alphas_cumprod_t * coords + sqrt_one_minus_alphas_cumprod_t * noise
            cond_input = self._maybe_apply_condition_dropout(cond)
            
            # 预测
            pred_noise = self.net(noised_coords, beta=beta, cond=cond_input) # (B, N, 3)
            ...
            condition_force_loss = self._compute_condition_force_loss(
            ...
            total_loss += (
                (loss + self.condition_force_weight * condition_force_loss) * (1.0 / self.interval_nums)
            )
    ...
    def get_loss_withCD(self, coords, cond, weights=None, ts=None, cd_weight=0.1):
        ...
        for i in range(self.interval_nums):
            ...
            total_loss += (
                (current_iter_loss + self.condition_force_weight * condition_force_loss)
                * (1.0 / self.interval_nums)
            )
