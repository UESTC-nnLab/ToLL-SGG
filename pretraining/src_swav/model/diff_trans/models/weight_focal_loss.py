...
def compute_adaptive_weight(points, k=16, base_scale=1.0, 
                            shape_boost=2.0, var_sensitivity=10.0):
    """
    计算自适应权重并映射到 [1, 2] 区间：
    1. 形状越复杂 -> 权重趋近 2.0
    2. 分布越发散(方差大) -> 权重趋近 1.0
    
    Args:
        points: (B, N, 3)
        k: 局部邻域大小
        base_scale: (原base_weight) 现在控制映射函数的敏感度/斜率。
                    值越大，权重越容易达到2.0；值越小，权重分布越平缓。
        shape_boost: 形状增益
        var_sensitivity: 方差敏感度
        
    Returns:
        weights: (B,) range in [1.0, 2.0)
    """
    B, N, C = points.shape
    device = points.device
    
    # ==========================================
    # Part 1: 计算全局方差 (Global Variance)
    # ==========================================
    global_variance = points.var(dim=1, unbiased=False).sum(dim=-1)
    
    # 方差因子 (范围 0.0 ~ 1.0)
    # 方差越大，exp(-x) 越接近 0
    variance_factor = torch.exp(-var_sensitivity * global_variance)
    
    # ==========================================
    # Part 2: 计算局部几何复杂度 (Local Complexity)
    # ==========================================
    # --- 2.1 KNN & Grouping ---
    dist_mat = torch.cdist(points, points)
    _, knn_indices = torch.topk(dist_mat, k=k, dim=-1, largest=False)
    
    knn_indices_flat = knn_indices.view(B, N * k)
    ...
    # 复杂度因子 (范围 >= 1.0)
    complexity_factor = 1.0 + shape_boost * (object_complexity * 10.0)

    # ==========================================
    # Part 3: 融合并映射到 [1, 2]
    # ==========================================
    # 1. 计算原始得分 (Raw Score)
    #    逻辑：方差是"门控"，如果方差太大(variance_factor -> 0)，raw_score 就会变成 0
    raw_score = base_scale * variance_factor * complexity_factor
    
    # 2. 使用 Tanh 进行软映射
    #    math: y = 1 + tanh(x)
    #    x >= 0 (因为 factor 都是正数)
    #    x = 0 (极差) -> tanh(0) = 0 -> weight = 1.0
    #    x -> inf (极好) -> tanh(inf) = 1 -> weight = 2.0
    final_weights = 1.0 + torch.tanh(raw_score)
