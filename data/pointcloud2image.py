...
from data.proc_zipfile import process_compressed_files
...
def read_pointcloud(scan_id):
    ...
    """
    plydata = trimesh.load(os.path.join('/home/honsen/tartan/msg_data/3rscan', scan_id, 'labels.instances.annotated.v2.ply'), process=False)
...
def get_label(label_path):
    ...
    text = torch.cat([clip.tokenize(f"a photo of a {c}") for c in label_list]).cuda() #(160,77)
    with torch.no_grad():
...
def read_json(split):
    ...
    selected_scans = set()
    if split == 'train' :
        selected_scans = selected_scans.union(read_txt_to_list('/home/honsen/honsen/CVPR2023-VLSAT/data/3DSSG_subset/train_scans.txt'))
        with open("/home/honsen/honsen/CVPR2023-VLSAT/data/3DSSG_subset/relationships_train.json", "r") as read_file:
            data = json.load(read_file)
    elif split == 'val':
        selected_scans = selected_scans.union(read_txt_to_list('/home/honsen/honsen/CVPR2023-VLSAT/data/3DSSG_subset/validation_scans.txt'))
        with open("/home/honsen/honsen/CVPR2023-VLSAT/data/3DSSG_subset/relationships_validation.json", "r") as read_file:
            data = json.load(read_file)
    else:
...
def read_scan_info(scan_id, mode='rgb'):
    scan_path = os.path.join("/home/honsen/tartan/msg_data/3rscan", scan_id)
...
def map_pc_to_image(points, instances, image_list, instance_names, extrinsics, intrinsic, width, height, class_list, class_weight, save_path, scene_id, fin_all=None, topk=10):
    """
    ...
    for i in instance_id:
        ...
        ax.scatter(points_i[:, 0], points_i[:, 1], points_i[:, 2], c='k', s=1)
        ax.set_xlabel('x (m)')
        ax.set_zlabel('z (m)')
        ax.set_ylabel('y (m)')
        plt.show()
        ...
        for k in topk_index:
            ...
            # plt.imsave("/home/honsen/qwe.jpg",croped_image_i)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--mode', type=str, default='val', help='train or test')
    args = argparser.parse_args()
    print("========= Deal with {} ========".format(args.mode))
    scene_data, selected_scans = read_json(args.mode)

    # process_compressed_files(selected_scans)

    # record global quanlity
    # fin_all = open(os.path.join(f'/data/3RScan/{args.mode}_all_quanlity.txt'), 'a')
    for i in tqdm(selected_scans):

        i = 'ab835faa-54c6-29a1-9b55-1a5217fcba19'
        ...
        save_path = f'/home/honsen/tartan/msg_data/3rscan/{i}/multi_view'
        os.makedirs(save_path, exist_ok=True)
        # print(f'======= map pointcloud to image =========')
        class_list, class_weight = get_label('/home/honsen/honsen/CVPR2023-VLSAT/data/3DSSG_subset/classes.txt')
        map_pc_to_image(pc_i, instances_i, image_list, instance_names, extrinsic_list, intrinsic_info['m_intrinsic'], intrinsic_info['m_Width'], intrinsic_info['m_Height'], class_list, class_weight, save_path, i)
    # fin_all.close()