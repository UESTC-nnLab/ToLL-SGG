...
def visualize_with_gt(feature_tensor: torch.Tensor, 
                      ...
                      pca_dim: int = 64,
                      class_names: list = None,
                      ignore_zero_label: bool = False): # [新增参数] 是否过滤 label 0
    """
    使用真值标签 (Ground Truth) 对特征进行 t-SNE 可视化。
    
    Args:
        ignore_zero_label: (bool) 如果为 True，将移除所有 label 为 0 的数据点 (通常用于过滤背景类边关系)
    """
    ...
    # --- [关键修改] 过滤 Label 为 0 的类别 ---
    if ignore_zero_label:
        print("Filtering out data with label 0 (Background/None)...")
        # 创建掩码：保留 label 不等于 0 的数据
        mask = labels_np != 0
        ...
        if filtered_count == 0:
            print("Error: No data left after filtering label 0! Skipping visualization.")
            return
    ...
    plt.title(f"{title_prefix} Feature Distribution\n(Colored by GT Labels | Zero Filtered: {ignore_zero_label})", fontsize=16)
    ...
    if class_names is not None:
        try:
            # 这里的逻辑是：如果 class_names 包含了 0 (None)，
            # 过滤后 labels_np 里没有 0 了，刻度会自动对齐到剩下的 label ID
            current_names = [class_names[i] for i in unique_labels if i < len(class_names)]
            if len(current_names) == len(unique_labels):
                cbar.set_ticklabels(current_names)
        except IndexError:
            pass
