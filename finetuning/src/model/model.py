...
class Pdiff4SSG_Pretraining():
    ...
    def data_processing_train_pdiff(self, items):
        if len(items) == 10:
            obj_points, obj_points_spatial, descriptor, edge_indices, anchor_ids, _obj_points_view2, _descriptor_view2, cur_obj_texts, batch_ids, obj_labels = items
        elif len(items) == 9:
            obj_points, obj_points_spatial, descriptor, edge_indices, anchor_ids, _obj_points_view2, _descriptor_view2, cur_obj_texts, batch_ids = items
            obj_labels = None
        elif len(items) == 8:
        ...
        return obj_points, descriptor, edge_indices, anchor_ids, batch_ids, obj_points_spatial, cur_obj_texts, obj_labels
    ...
    def train(self):
        ...
        if init_weights_path and os.path.exists(init_weights_path):
            ...
            try:
                ...
                print("[Info] Loaded initialization weights with strict=True.")
            except Exception as e:
                print(f"[Warning] strict=True init load failed, will try strict=False. Error: {e}")
                incompatible = self.model.load_state_dict(normalized_state_dict, strict=False)
                print(f"[Warning] Loaded initialization weights with strict=False. Missing: {len(incompatible.missing_keys)}, Unexpected: {len(incompatible.unexpected_keys)}")

            print("[Info] Training will start from epoch 0 with fresh optimizer and scheduler state.")

        elif init_weights_path:
            print(f"Warning: INIT_WEIGHTS_PATH '{init_weights_path}' was specified but file not found. Falling back to resume logic.")
        ...
        while(keep_training):
            ...
            for batch_idx, items in enumerate(pbar):
                ...
                obj_points, descriptor, edge_indices, anchor_ids, batch_ids, obj_points_spatial, cur_obj_texts, obj_labels = self.data_processing_train_pdiff(items)
