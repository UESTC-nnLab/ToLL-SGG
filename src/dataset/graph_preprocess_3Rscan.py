...
def preprocess_scan_scene(scan_id: str, data_path: str):
    ...
    try:
        K_MIN = 4  # 每个子图的最小尺寸
        # 加载点云数据
        results = load_mesh(data_path,'labels.instances.annotated.v2.ply',False,False)
       
        point_cloud = results['points'] # Nx3
        object_labels = results['instances']  # N
...
def preprocess_scanNet_main(ScanNet_dirs):
    ...
    scan_lists = os.listdir(ScanNet_dirs)[0:20]
...
if __name__ == '__main__':
    dataset_dirs = '/home/honsen/tartan/SGG_dataset/3RScan' 
    preprocess_scanNet_main(dataset_dirs)