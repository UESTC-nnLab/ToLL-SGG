...
class Mmgnet(BaseModel):
    ...
    def process_val(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, use_triplet=False):
        ...
        precls = evaluate_triplet_recallk(obj_logits_3d, rel_cls_3d, gt_edges, edge_indices, True, [20, 50, 100], 50, evaluate='rels')
        precls_m = evaluate_triplet_mrecallk(obj_logits_3d, rel_cls_3d, gt_edges, edge_indices, True, [20, 50, 100], 50,
                                       evaluate='rels')

        sgcls = evaluate_triplet_recallk(obj_logits_3d, rel_cls_3d, gt_edges, edge_indices, True, [20, 50, 100], 50)
        sgcls_m = evaluate_triplet_mrecallk(obj_logits_3d, rel_cls_3d, gt_edges, edge_indices, True, [20, 50, 100], 50)

        top_k_rel, scene_accuracy = evaluate_topk_predicate(rel_cls_3d.detach().cpu(), gt_edges, self.mconfig.multi_rel_outputs, topk=6)

        top_k_obj_2d = evaluate_topk_object(obj_logits_2d.detach().cpu(), gt_cls, topk=11)
        top_k_rel_2d, _ = evaluate_topk_predicate(rel_cls_2d.detach().cpu(), gt_edges, self.mconfig.multi_rel_outputs, topk=6)
        ...
        return top_k_obj, top_k_obj_2d, top_k_rel, top_k_rel_2d, top_k_triplet, top_k_2d_triplet, cls_matrix, sub_scores, obj_scores, rel_scores,scene_accuracy, precls, precls_m, sgcls, sgcls_m
